...
"# allows any local user to connect as any SakthiDB user, including\n" \
...
bootstrap_template1(void)
{
	...
	snprintf(headerline, sizeof(headerline), "# SakthiDB %s\n",
			 PG_MAJORVERSION);

	if (strcmp(headerline, *bki_lines) != 0)
	{
		pg_log_error("input file \"%s\" does not belong to SakthiDB %s",
					 bki_file, PG_VERSION);
...
{
	printf(_("%s initializes a SakthiDB database cluster.\n\n"), progname);
	printf(_("Usage:\n"));
							   ...
							   backend_exec)) < 0)
	{
		...
		if (ret == -1)
			pg_fatal("program \"%s\" is needed by %s but was not found in the same directory as \"%s\"",
					 "SakthiDB", progname, full_path);
		else
			pg_fatal("program \"%s\" was found by \"%s\" but was not the same version as %s",
					 "SakthiDB", full_path, progname);
	}
...
setup_data_file_paths(void)
{
	...
	if (show_setting || debug)
	{
		fprintf(stderr,
				"VERSION=%s\n"
				"PGDATA=%s\nshare_path=%s\nPGPATH=%s\n"
				"SakthiDB_SUPERUSERNAME=%s\nSakthiDB_BKI=%s\n"
				"SakthiDB_CONF_SAMPLE=%s\n"
				"PG_HBA_SAMPLE=%s\nPG_IDENT_SAMPLE=%s\n",
				PG_VERSION,
