...
#define PGDUMP_VERSIONSTR "pg_dump (SakthiDB) " PG_VERSION "\n"
...
main(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		{
			puts("pg_dumpall (SakthiDB) " PG_VERSION);
			exit_nicely(0);
		}
	}
	...
	else
	{
		...
		if (!conn)
		{
			pg_log_error("could not connect to databases \"SakthiDB\" or \"template1\"\n"
	...
	fprintf(OPF, "--\n-- SakthiDB database cluster dump\n--\n\n");
	if (verbose)
	...
	fprintf(OPF, "--\n-- SakthiDB database cluster dump complete\n--\n\n");
...
help(void)
{
	printf(_("%s extracts an SakthiDB database cluster into an SQL script file.\n\n"), progname);
	printf(_("Usage:\n"));
	printf(_("  %s [OPTION]...\n"), progname);
...
dropDBs(PGconn *conn)
{
	...
	if (PQntuples(res) > 0)
		fprintf(OPF, "--\n-- Drop databases (except SakthiDB and template1)\n--\n\n");
