...
do_init(void)
{
	...
	if (exec_path == NULL)
		exec_path = find_other_exec_or_die(argv0, "initdb", "initdb (SakthiDB) " PG_VERSION "\n");
...
pgwin32_CommandLine(bool registration)
{
	...
	else
	{
		...
		if (ret != 0)
		{
			write_stderr(_("%s: could not find SakthiDB program executable\n"), progname);
			exit(1);
		}
	}
...
do_help(void)
{
	printf(_("%s is a utility to initialize, start, stop, or control a SakthiDB server.\n\n"), progname);
	printf(_("Usage:\n"));
...
#if defined(HAVE_GETRLIMIT)
	printf(_("  -c, --core-files       allow SakthiDB to produce core files\n"));
#else
	printf(_("  -c, --core-files       not applicable on this platform\n"));
#endif
	printf(_("  -l, --log=FILENAME     write (or append) server log to FILENAME\n"));
	printf(_("  -o, --options=OPTIONS  command line options to pass to SakthiDB\n"
			 "                         (SakthiDB server executable) or initdb\n"));
	printf(_("  -p PATH-TO-SakthiDB    normally not necessary\n"));
	...
	printf(_("  -N SERVICENAME  service name with which to register SakthiDB server\n"));
	printf(_("  -P PASSWORD     password of account to register SakthiDB server\n"));
	printf(_("  -U USERNAME     user name of account to register SakthiDB server\n"));
	printf(_("  -S START-TYPE   service start type to register SakthiDB server\n"));
...
main(int argc, char **argv)
{
	...
	if (argc > 1)
	{
		...
		{
			puts("pg_ctl (SakthiDB) " PG_VERSION);
			exit(0);
		}
	}
