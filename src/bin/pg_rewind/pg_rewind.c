...
{
	printf(_("%s resynchronizes a SakthiDB cluster with another copy of the cluster.\n\n"), progname);
	printf(_("Usage:\n  %s [OPTION]...\n\n"), progname);
	printf(_("Options:\n"));
...
main(int argc, char **argv)
{
	...
	if (geteuid() == 0)
	{
		...
		pg_log_error_hint("You must run %s as the SakthiDB superuser.",
						  progname);
		exit(1);
	}
#endif
...
ensureCleanShutdown(const char *argv0)
{
	...
	{
		pg_log_error("SakthiDB single-user mode in target cluster failed");
