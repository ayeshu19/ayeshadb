...
DllRegisterServer(void)
{
	...
	{
		MessageBox(NULL, "Could not retrieve DLL filename", "SakthiDB error", MB_OK | MB_ICONSTOP);
	...
	{
		MessageBox(NULL, "Could not create the registry key.", "SakthiDB error", MB_OK | MB_ICONSTOP);
		return SELFREG_E_TYPELIB;
	}
	...
	if (RegSetValueEx(key,
					  ...
					  strlen(buffer) + 1))
	{
		MessageBox(NULL, "Could not set the event message file.", "SakthiDB error", MB_OK | MB_ICONSTOP);
		return SELFREG_E_TYPELIB;
	}
	...
	if (RegSetValueEx(key,
					  ...
					  sizeof(DWORD)))
	{
		MessageBox(NULL, "Could not set the supported types.", "SakthiDB error", MB_OK | MB_ICONSTOP);
		return SELFREG_E_TYPELIB;
	}
...
DllUnregisterServer(void)
{
	...
	{
		MessageBox(NULL, "Could not delete the registry key.", "SakthiDB error", MB_OK | MB_ICONSTOP);
		return SELFREG_E_TYPELIB;
	}
	return S_OK;
}
